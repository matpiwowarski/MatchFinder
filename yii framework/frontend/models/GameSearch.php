...
class GameSearch extends Game
{
    ...
    public function search($params)
    {
        ...
        public function getGame($UserLongitude, $UserLangitude)
        {
            ...
            $query->select('game.id, stadium.latitude, stadium.longitude, stadium.street, stadium.number, team.id, team_currentForm.id, team_currentForm.position, team_currentForm.games_played, team_currentForm.wins, team_currentForm.draws, 
            ...
            $query->from('game')->leftJoin('stadium','stadium.id = game.Stadium_id')->leftJoin('team', 'game.Team_home_id=team.id OR game.Team_away_id=team.id')
            ->leftJoin('team_currentForm', 'team.Team_currentForm_id=team_currentForm.id');
    ...
    function teamLastGamesForm($length, $arrayTeamCurrentForm){
    ...
    function teamAttractiveness($arrayTeamCurrentForm)
        {
            ...
            for($i=0; $i<$length; $i++)
            {
                $arrayTeamCurrentForm[$i]['teamCurrentFormValue']=($arrayTeamCurrentForm[$i]['teamPostionValue'] + $arrayTeamCurrentForm[$i]['lastGamesValue'] +
                $arrayTeamCurrentForm[$i]['winsPercentegeValue']+$arrayTeamCurrentForm[$i]['goalsScoredValue']);
            }
    ...
    function distanceValue($length, $arrayTeam){

        for($i=0;$i<$length;$i++)
        {
            $distanceValue=0;
            
            $distance=$arrayTeam[$i]['distance'];
    ...
    function gamesValue($sortedArray, $UserLongitude, $UserLangitude)
    {
        ...
        $sortedArray=distanceValue($length, $sortedArray);
        ...
        array_multisort($gameValue, $sortedArray);
